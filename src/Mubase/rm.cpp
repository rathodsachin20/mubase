...
int Schema::getAttrIndex(const char* name){
    for(int i=0; i<numAttrs; i++){
        if(strcmp(name, attrName[i])==0)
            return i;
    }
    return -1;
}
...
Schema* Schema::concateSchema(Schema* sch, Schema* newSchema){
    ...
    for(int i=0; i<lnum; i++){
        ...
        attrName[i] = this->attrName[i];
    }
    for(int i=0; i<rnum; i++){
        attrType[i + lnum] = sch->attrType[i];
        sizeAttrs[i + lnum] = sch->sizeAttrs[i];
        attrName[i+ lnum] = sch->attrName[i];
    }
...
void Record::print() {
    if (schema->getAttrType(0) == 1) {
            cout << getInt(0);
        } else if (schema->getAttrType(0) == 2) {
            cout << getFloat(0);
        } else if (schema->getAttrType(0) == 3) {
            cout << getConstString(0);
        } else if (schema->getAttrType(0) == 4) {
            cout << getVarString(0);
        }
        
    for (int i = 1; i < schema->getNumAttrs(); i++) {
...
//cout<<"in readschema for "<<relName1<<endl;
    Schema* relSchema;
    int relArity = 0;
    short int* attrType;
    char **attrName;
    ...
    int totSOSlots = sosp.getTotalNoOfSlots();
    for (int i = 0; i < totSOSlots; i++) {
    ...
    if(!relExists){
        cout<<"Relation "<<relName1<<" does not exist.Exiting.\n";
        system("exit");
    }
    ...
    int totSCSlots = scsp.getTotalNoOfSlots();
    for (int i = 0; i < totSCSlots; i++) {
