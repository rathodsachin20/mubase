...
char *SlottedPage::getRecord(short slotNum, int& l) {
...
short SlottedPage::storeRecord(char *record) {
    cout<<" In sp.storeRecord :";
    cout<<" got record :'"<<record<<"'over"<<endl;
 ...
 */
short SlottedPage::storeRecord(char* record, int length) {
    //cout<<" In sp.storeRecord :length = "<<length<<endl;
    //cout<<" got record :'"<<record<<"'over"<<endl;

    //cout<<"REBYTE IN SP INSERT = "<<readShort(record,0)<<"  "<<readShort(record, 2)<<"  "<<readInt(record, 5)<<"  "<<readInt(record, 9)<<endl;
    ...
    writeShort(block, recSlot, newEndOfFreeSpace);
    writeShort(block, recSlot + sizeof (short), recLength);
    writeBytes(block, (int) newEndOfFreeSpace, record, recLength);

    //short tempOffset = readShort(block,recSlot);
    //cout<<"  tempOffset="<<tempOffset<<"  recLength="<<recLength<<endl;
    //char* rec = readBytes(block, tempOffset, recLength);
    //cout<<"REBYTE IN SPbbbbbbbbb = "<<readShort(block,tempOffset)<<"  "<<readShort(block, tempOffset+2)<<"  "<<readInt(block,tempOffset+ 17)<<"  "<<readBytes(block,tempOffset+21,4)<<endl;
